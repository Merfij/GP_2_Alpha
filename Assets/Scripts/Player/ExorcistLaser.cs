...
public class ExorcistLaser : MonoBehaviour
{
    ...
    public void ActivateLaser()
    {
        ...
        laserGate = true;
        EventReference evt = AudioManager.Instance.GetEvent("ExorcistBeam");
    ...
    void Update()
    {
        if (laserBeam.enabled)
        {
            ...
            if (Physics.Raycast(cam.position, cam.forward, out hit, 100f, combinedMask, QueryTriggerInteraction.Ignore)) //if the raycast hits something
            {
                ...
                else { ClearTarget(); } //raycast hits something that does not implement IAbilityTarget
